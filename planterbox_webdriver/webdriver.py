...
@step('I click "(.*?)"$')
def click(test, name):
    elem = test.world.browser.find_element_by_link_text(name)
    elem.click()


@step('I should see a link with the url "(.*?)"$')
def should_see_link(test, link_url):
    test.assertTrue(test.world.browser.find_element_by_xpath(
        u'//a[@href="%s"]' % link_url
    ))
...
def element_not_contains(test, element_id, value):
    elem = test.world.browser.find_elements_by_xpath(
        u'id("{id}")[contains(., "{value}")]'.format(
            id=element_id, value=value,
        )
    )
...
def should_see_id_in_seconds(test, element_id, timeout):
    elem = wait_for_visible_elem(test.world.browser,
                                 u'id("%s")' % element_id,
                                 timeout=int(timeout),
                                 )
...
def should_see_id(test, element_id):
    elem = test.world.browser.find_element_by_xpath(
        u'id("%s")' % element_id
    )
...
def should_see_in_seconds(test, text, timeout):
    test.assertTrue(wait_for_content(
        test.world.browser, text, timeout=int(timeout)
    ))


@step('I should see "([^"]+)"$')
def should_see(test, text):
    test.assertTrue(contains_content(test.world.browser, text))
...
def url_should_contain(test, url):
    test.assertIn(url, test.world.browser.current_url)


@step('The browser\'s URL should not contain "(.*?)"$')
def url_should_not_contain(test, url):
    test.assertNotIn(url, test.world.browser.current_url)
...
def fill_in_textfield(test, field_name, value):
    date_field = find_any_field(test.world.browser,
                                DATE_FIELDS,
                                field_name)
    if date_field:
        field = date_field
    else:
        field = find_any_field(test.world.browser,
                               TEXT_FIELDS,
                               field_name)
...
def press_button(test, value):
    button = find_button(test.world.browser, value)
    button.click()
...
def input_has_value(test, field_name, value):
    ...
    """
    text_field = find_any_field(test.world.browser,
...
def submit_the_only_form(test):
    ...
    """
    form = test.world.browser.find_element_by_xpath(str('//form'))
    form.submit()
...
def submit_form_id(test, id):
    ...
    """
    form = test.world.browser.find_element_by_xpath(
        u'id("{id}")'.format(id=id)
    )
    form.submit()
...
def check_checkbox(test, value):
    check_box = find_field(test.world.browser, 'checkbox', value)
...
def select_single_item(test, option_name, select_name):
    option_box = find_option(test.world.browser, select_name, option_name)
    option_box.click()
...
def select_contains(test, option, id_):
    test.assertIsNotNone(option_in_select(test.world.browser, id_, option))
...
def choose_radio(test, value):
    box = find_field(test.world.browser, 'radio', value)
    box.click()
...
def accept_alert(test):
    ...
    try:
        alert = Alert(test.world.browser)
        alert.accept()
...
def press_by_tooltip(test, tooltip):
    ...
    Press a button having a given tooltip.
    """
    for button in test.world.browser.find_elements_by_xpath(
        u'//*[@title="%(tooltip)s" or @data-original-title="%(tooltip)s"]' %
        dict(tooltip=tooltip)
    ):
        try:
...
def page_title(test, title):
    ...
    test.assertEqual(test.world.browser.title, title)


@step(r'I switch to the frame with id "([^"]*)"')
def switch_to_frame(test, frame):
    elem = test.world.browser.find_element_by_id(frame)
    test.world.browser.switch_to_frame(elem)


@step(r'I switch back to the main view')
def switch_to_main(test):
    test.world.browser.switch_to_default_content()
